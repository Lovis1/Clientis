...
class CsvToJsonMuPo extends React.Component {
  //PROPS
  // setTestState;
  constructor(props) {
    super(props);
  ...
  saveToGraphCMS = async () => {
    ...
    mutation MyMutation($data:Json) {
        createImportJSON(data: {importJSON: $data, target:true}) {
          id
          createdAt
          importJSON
          target
    ...
    try {
      ...
      console.log(data);
      this.state.importDate = data.data.createImportJSON.createdAt;
    } catch (error) {
      console.error(error);
    }
    // setTestState("HI WILL THIS WORK?");
